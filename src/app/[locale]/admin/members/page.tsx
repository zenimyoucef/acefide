  ...
  return (
    <div>
      ...
      <div className="space-y-6">
        <div className="overflow-x-auto rounded-2xl border bg-white shadow-sm"><table className="w-full min-w-[760px] text-sm"><thead className="bg-[#f8faf7] text-start"><tr><th className="p-4">Applicant</th><th className="p-4">Email</th><th className="p-4">Phone</th><th className="p-4">Created At</th><th className="p-4">Current Status</th><th className="p-4">Actions</th></tr></thead><tbody>{items.map((item) => <tr key={item.id} className="border-t"><td className="p-4 font-semibold">{item.name}</td><td className="p-4">{item.email}</td><td className="p-4">{item.phone || "—"}</td><td className="p-4">{item.createdAt.toLocaleDateString(locale)}</td><td className="p-4"><StatusBadge status={item.status} /></td><td className="p-4"><div className="flex flex-nowrap items-center gap-2">{item.status === "PENDING_REVIEW" && <><AdminForm action={approveMembershipRequest.bind(null, locale, item.id)} locale={locale}><button className="whitespace-nowrap rounded-lg bg-primary/10 px-3 py-1.5 text-xs font-bold text-primary hover:bg-primary/20">Approve</button></AdminForm><AdminForm action={rejectMembershipRequest.bind(null, locale, item.id)} locale={locale} confirmMessage={`Reject ${item.name}'s application?`}><button className="whitespace-nowrap rounded-lg bg-red-50 px-3 py-1.5 text-xs font-bold text-red-700 hover:bg-red-100">Reject</button></AdminForm></>}<DeleteApplicantButton action={deleteMembershipRequest.bind(null, locale, item.id)} applicantName={item.name} locale={locale} /></div></td></tr>)}</tbody></table></div>
        {items.map((item) => {
