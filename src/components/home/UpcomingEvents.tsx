...
const activities = [
  { slug: "african-business-fair-2026", date: "08.06.2026", type: "مشاركة", title: "المشاركة في الطبعة الرابعة للصالون الإفريقي للأعمال", location: "المدرسة العليا للفندقة والإطعام، الجزائر" },
  { slug: "mining-innovation-roadmap-2025", date: "30.07.2025", type: "لقاء تنسيقي", title: "خارطة طريق للابتكار والمقاولاتية في قطاع المناجم", location: "مقر المديرية العامة للبحث العلمي، الجزائر" },
  { slug: "sonarem-mining-cooperation-2025", date: "02.06.2025", type: "فعالية وشراكة", title: "المناجم: الاستثمار والابتكار والتحويل المحلي", location: "الجزائر العاصمة" },
] as const;

const copy = { kicker: "الحضور الميداني", title: "أبرز الأنشطة", body: "لقاءات تجمع المؤسسات والخبراء وصنّاع الفرص حول مستقبل الاقتصاد والاستثمار.", all: "كل الأنشطة", featured: "فعالية بارزة" };
      ...
      <div className="container-content">
        ...
        <div className="mb-14 flex items-end justify-between gap-6">
          <div className="max-w-2xl">
            ...
            <p className="section-copy mt-5">{copy.body}</p>
          </div>
        ...
        <div className="grid gap-5 lg:grid-cols-[1.2fr_.8fr]">
          ...
          <Link href={`/events/${featured.slug}`} className="group relative flex min-h-[360px] flex-col justify-end overflow-hidden rounded-[1.75rem] bg-[#0b1f33] p-7 text-white shadow-[0_22px_60px_rgba(11,31,51,.16)] transition-all duration-500 hover:shadow-[0_28px_70px_rgba(11,31,51,.24)] hover:-translate-y-1 md:p-10">
            ...
            <div className="relative">
              <span className="inline-flex rounded-full border border-white/15 bg-white/10 px-3 py-1 text-xs font-bold uppercase tracking-wider text-[#e5c978] backdrop-blur-sm">{copy.featured}</span>
              ...
              <Link key={item.slug} href={`/events/${item.slug}`} className="group flex min-h-[170px] flex-col justify-between rounded-2xl border border-border/40 bg-[#f9faf8] p-6 transition-all duration-400 hover:-translate-y-1 hover:border-primary/20 hover:bg-white hover:shadow-[0_12px_35px_rgba(11,31,51,0.08)]">
                <div>
                  <div className="flex items-center justify-between gap-4">
                    <span className="rounded-full bg-primary/8 px-3 py-1 text-xs font-bold text-primary transition-colors duration-300 group-hover:bg-primary/12">{item.type}</span>
                    <span className="text-xs font-bold text-accent">{item.date}</span>
                  </div>
